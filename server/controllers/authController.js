...
const sendTokenResponse = (user, sessionId, statusCode, res) => {
  ...
  // [CRITICAL FIX] Determine Environment
  // Render sets NODE_ENV=production by default.
  // Localhost usually defaults to 'development' or undefined.
  const isProduction = process.env.NODE_ENV === "production";

  const cookieOptions = {
    expires: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days
    httpOnly: true,
    // If Production (Render): Secure = true (HTTPS)
    // If Development (Localhost): Secure = false (HTTP)
    secure: isProduction,
    // If Production (Vercel -> Render): SameSite = 'none' (Cross-Site)
    // If Development (Localhost): SameSite = 'lax' (Browser default, easier for local)
    sameSite: isProduction ? "none" : "lax",
  };

  // Debug Log to help you verify what's happening
  console.log(
    `🍪 Setting Cookies | Env: ${process.env.NODE_ENV || "dev"} | Secure: ${cookieOptions.secure} | SameSite: ${cookieOptions.sameSite}`,
  );
